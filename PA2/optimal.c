...
void combineOptions(Node * node, int * check) {
    ...
    if(node->label == -1) {
        ...
        node->dims = malloc(maxPossible * sizeof(int*));
        if(node->dims == NULL) {
        ...
        for(int i = 0; i < maxPossible; i++) {
            node->dims[i] = malloc(4* sizeof(int)); //left 2 are for width and height lengths, right 2 are for index of the rep (left, right)
            if(node->dims[i] == NULL) {
                for(int j = 0; j < i; j++) {
                    free(node->dims[j]);
                }
        ...
        if(!divType) { //V node
            ...
            int ** temparr = malloc(maxPossible * sizeof(int*)); //array that holds the values that will be reversed
            if(temparr == NULL) {
                *check = 1;
