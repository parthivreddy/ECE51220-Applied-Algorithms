...
float MCM(char * filename, headNode ** adjList, int numEdges, int numNodes, int * checkValid) {
    // double ** dTab = distanceTable(numNodes, numEdges);
    // int ** pTab = predTable(numNodes, numEdges);
    double ** dTab = malloc((numNodes + 1) * sizeof(double *));
    int ** pTab = malloc((numNodes + 1) * sizeof(int *));
    dTab[0] = malloc(numNodes * sizeof(double));
    pTab[0] = malloc(numNodes * sizeof(int));
    ...
    for(int k = 1; k <= numNodes; k++) {
        //now for each edge from each node
        //(k - 1) is source node
        dTab[k] = malloc(numNodes * sizeof(double));
        pTab[k] = malloc(numNodes * sizeof(int));
        for(int v = 0; v < numNodes; v++) {
            dTab[k][v] = INFINITY;
            pTab[k][v] = __INT_MAX__;
        }
