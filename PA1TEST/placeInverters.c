...
double calcMaxWrLen(Node * curr, double * invSpecs, double * wrSpecs, double sinkCap, double maxDelay, Node * root, int * checkStringency) {
    double del = 0;
    ...
    double b = wrSpecs[1]*(curr->sinkCapacitance + invSpecs[1] + invSpecs[2]*a);
    ...
    if(d < 0) { //just insert inverters directed at both children
        fprintf(stderr, "Generating imaginary wrLength!\n");
        *checkStringency = 1;
        return EXIT_FAILURE;
    }
    else {
        ...
        if(wrLen1 <= 0 && wrLen2 <= 0) {
            fprintf(stderr, "Generating negative or 0 wrLength!\n");
            *checkStringency = 1;
            return EXIT_FAILURE;
        }
    }
